...
const Sidebar: React.FC = () => {
  ...
  const criticalStocksCount = ingredients.filter(
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { id: 'session', label: 'Cloture (Z)', icon: Moon, role: 'MANAGER' },
      ],
    },
    {
      title: 'GESTION METIER',
      items: [
        {
          id: 'menu',
          label: 'Menu & Recettes',
        ...
        },
        {
          ...
          icon: Package,
          role: 'MANAGER',
          badge: criticalStocksCount > 0 ? criticalStocksCount : undefined,
        },
        {
          id: 'customers',
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'expenses', label: 'Depenses', icon: Wallet, role: 'MANAGER' },
        { id: 'settings', label: 'Parametres', icon: Settings, role: 'MANAGER' },
      ],
    },
  ];
    ...
    <aside className="w-64 bg-[#b3247e] text-white flex flex-col h-screen overflow-hidden shrink-0 shadow-2xl z-30">
      <div className="p-8 flex items-center gap-3 border-b border-white/10">
        ...
        <div className="flex flex-col">
          <span className="font-black text-xl leading-none tracking-tighter truncate w-32">
            {truckSettings.name.split(' ')[0]}
          </span>
          <span className="text-[10px] font-bold opacity-60 uppercase tracking-[0.2em]">
            {truckSettings.name.split(' ').slice(1).join(' ') || 'Manager'}
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-8 overflow-y-auto scrollbar-hide mt-4">
        {sections.map((section) => {
          const visibleItems = section.items.filter((item) =>
            hasPermission(item.role as any),
          );
          if (visibleItems.length === 0) return null;

          return (
            <div key={section.title}>
              <p className="px-4 text-[10px] font-black text-white/40 mb-4 tracking-[0.3em] uppercase">
                {section.title}
              </p>
